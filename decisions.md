...
### DEC-DASH-01 · Dashboard dueCount vs session count — known remaining gap

**Status:** Partially fixed. Still diverges on language/collection exclusions.
...
### DEC-SRS08 · Session cap source of truth + "Review more" UX — IMPLEMENTED 2026-06-06
...
**1 — `ind_profiles.preferences.daily_cap` is the source of truth.**
...
**2 — Session cap formula:** `sessionCap = max(0, cap − reviewedToday)`. Custom sessions (`?custom=1`) and "Review more" sessions (`?more=1`) bypass this. "Review more" loads one cap-sized batch (not the full queue).

**3 — Dashboard "Review more?" state:** Three states, not two:
...
**4 — All OptionsSheet settings sync to cloud.** Any setter that writes to `localStorage` must also call `patchPreferences(patch)`. `SettingsSheet` already did this; `ReviewSession.OptionsSheet` now does too (via `patchPreferences` from `lib/db/profile/preferences.ts`). See architecture.md Settings sync rule.
...
### DEC-SRS07 · Learning phase gesture + label design
