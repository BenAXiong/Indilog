...
|-----------|------|-------------|
| 2026-07-09 03:12 | FIX | `countLearnFlashcards`'s retry (5841ce2) never actually retried — it re-awaited the same query builder instance, which resolved with the same error again without firing a second network request (confirmed via the timeline script: no second `→ REQUEST` line ever appeared). Rebuilt the query fresh for the retry via a local `buildQuery()` factory, matching `countDueFlashcards`'s already-working `buildDueQuery()` pattern. Verified `.signal` is only ever set via an explicit opt-in `.abortSignal()` call the app never uses, ruling out a shared-abort-state explanation |
| 2026-07-09 02:55 | FIX | `countDueFlashcards`/`countLearnFlashcards` now retry once on error before falling back to 0 — traced via the BUG-01 repro's request timeline: the landing-page count HEAD request can get `ERR_ABORTED` by the browser (unrelated to app code — postgrest-js 2.106.2 explicitly never retries aborted requests by design), which was silently rendering as "0 due"/"No new cards" even when cards existed. Confirmed postgrest-js's own retry logic doesn't cover this case before adding the manual retry |
| 2026-07-09 02:32 | FIX | Custom-deck detail page header undercounted cards/lessons on decks >1000 cards: `listCollectionCards` used `.limit(10000)`, but PostgREST silently caps returned rows at 1000 regardless of requested limit (same class as DEC-SRS04, previously hit in getStudyStats and listBrowserCards). Switched to the existing `paginate<T>` helper (batched `.range()` fetches) so the `{totalCards} cards · {totalLessons} lessons` counter and lesson groupings reflect the true deck size |
| 2026-07-09 01:45 | FIX | BUG-01 root cause found and fixed: `useSwipeGesture()` was called ~170 lines after the `if (!entry) return null` guard in both `LearnSession` and `ReviewSession`, so the natural-completion render (entry undefined for one pass before unmount) called fewer hooks than the prior render — React error #300, surfaced as "Application error" in prod. Moved both hook calls above their guards (`entry?.exposureDone`/`entry?.goodCount` fallbacks). Confirmed via a scripted authenticated Playwright repro (mint-session.mjs + driven Learn session) before and would confirm after redeploy; docs/bugs.md updated with root cause, now tracked in git |
| 2026-07-08 18:42 | FIX | Backfilled `metadata.curriculum_source` on the 283 pre-fix curriculum items (level IS NULL) via exact ab-text + matching-dialect join against corpus_sentences/corpus_occurrences, guarded by `having count(distinct source) = 1` so it only writes unambiguous matches; all 283 resolved cleanly (grmpts 187, essay 34, dialogue 32, con_practice 30). Initially assumed in DEC-SRS16 this wasn't reliably backfillable — wrong, hadn't actually checked; ADR updated. |
| 2026-07-08 18:23 | FEATURE | Priority-deck visibility: curriculum saves now record `metadata.curriculum_source` per Epark source (forward-only, no backfill) so `matchesPriorityDeck` can distinguish Conversations/Dialogues/Essays/Patterns instead of lumping them via a level-null heuristic; session header (`deckName`, review + learn) falls through to the card's real collection/note_source name instead of going blank for non-priority cards; Learn/Review session-size pickers (`DualRingCard`) gained a "Decks" tab showing due/new cards grouped by priority deck then non-priority collections by count, via new `getDeckBreakdown()` (client-side, reuses `matchesPriorityDeck` rather than a SQL port). `matchesPriorityDeck` refactored to a single object param (was 8 positional args, hit the lint ceiling). DEC-SRS16 (implemented, not yet manually verified — see ADR for pending test checklist). |
| 2026-07-08 17:34 | FIX | Review/learn/study language exclusion filters (`show_all_langs`/`excluded_langs`) now read from `ind_profiles.preferences` at query-build time instead of racing a synchronous `localStorage` read that could be stale on a device/tab that hadn't opened SettingsSheet yet; new `fetchReviewPrefsSnapshot()` (preferences.ts) is the single authoritative fetch; `getExcludeFromReview()` returns the lang fields too; review page's `reload()`/`loadSessionContext` share one exclusion fetch instead of up to three separate `ind_profiles` round trips; `ReviewSession`/`LearnSession` options-sheet state now seeded from `ctx` prop, not a second localStorage read. DEC-SRS15 (implemented, not yet manually verified — see ADR for pending test checklist). |
| 2026-07-08 12:58 | FIX | Unsave restored on curriculum + dict bookmarks — new `unsaveItem()` in `lib/db/srs/browser.ts` deletes the note if `repetitions === 0` (nothing to lose), else suspends the flashcard so review history/heatmap stay intact; two-outcome toast replaces the old "go to Browser to remove" warning; `EparkSentence`/`EparkView` `onSaveWarning` → `onUnsave`; dict page `savedAbSet` now a `Map<ab,id>`; dead `deleteItem()` in `notebook/items.ts` removed (DEC-SRS14) |
| 2026-07-08 12:23 | FEATURE | Study tab — Patterns deck shows an amber warning icon (tap/hover tooltip) when the user's selected dialect differs from the grmpts content's actual source dialect; grmpts is scraped once per language and shared across all sub-dialects for convenience (e.g. Amis grmpts is Xiuguluan-dialect content shown to all 5 Amis dialects). New GRMPTS_SOURCE_DIALECT map in dialects.ts, seeded with only the confirmed Amis→Xiuguluan mapping; tooltip names both the content's dialect and the user's |
| 2026-07-08 04:12 | CONFIG | DEC-SRS13 ADR (audio surfacing in all session modes); DEC-SRS06 Decision 2 amendment note + decisions.md index rows; learn-feature.md stale repairAudioUrl snippet flagged with pointer to curriculum.ts |
| 2026-07-08 01:55 | FEATURE | All session modes surface audio when present: forward/reverse autoplay+button on reveal (reverse prompt would leak the answer), sts autoplay+button on prompt, audio mode unchanged; Learn exposure passes autoplay on advance since the back is already visible; shared PlayChip in CardContent; architecture.md session-mode table updated |
...
| 2026-06-30 00:48 | REFACTOR | ePark priority decks: replace per-unit (level/lesson) approach with per-source (Lessons/Patterns/Essays/Dialogues/Conversations); collapsible ePark group in picker; filter_config now stores curriculum_source+label; matchesPriorityDeck uses level!=null as twelve proxy |
| 2026-06-30 00:48 | FIX | Backfill level/lesson/lesson_title for 77 curriculum ind_items (twelve/Lessons source) via corpus_occurrences JOIN; forward-fix EparkView.handleSave to write these fields for twelve source; add level/lesson/lesson_title to CreateItemInput |
| 2026-06-30 00:48 | FEATURE | Curriculum unit priority rows: filter_config JSONB on ind_priority_decks; matchesPriorityDeck helper; listCurriculumUnits/addCurriculumUnitDeck; GoalSheet picker shows "ePark units" section; review/learn pages use matchesPriorityDeck |
| 2026-06-30 00:48 | FEATURE | GoalWidget: compact 3-row layout — row 1: "Goal" + deck chain (→-linked, ≤80% width, ··· if more); row 2: maturity % counters + Sim/Manual pill; row 3: progress bar |
...
| 2026-06-19 04:36 | FEATURE | per-deck learn+review entry: collection detail page shows Learn/Review CTAs with live new/due counts; listLearnFlashcards accepts collectionId to filter + sort by level/lesson/position; learn page bypasses daily target cap when collectionId param present |
| 2026-06-19 05:30 | FEATURE | Study tab deck-entry improvements: (1) back arrows on review/learn landing pages now return to /study when launched from a collection/capture context (custom=1 or collectionId); (2) ePark tab gains a "Saved" row with due-count badge linking to review for all curriculum note_source items; (3) Captures tab expands into three sub-decks (Captured/Dict saves/Imports) each with their own due count, stop curriculum bleed; (4) listDueFlashcards gains includeNoteSource opt; capturesOnly now explicitly targets captured+dict+import only |
| 2026-06-19 06:30 | FEATURE | Dashboard session size override: pen icon next to active Learn/Review CTAs opens a stepper popup (±5, min 1/5, max newCount/totalDue); starts session with ?n= param; review/page.tsx reads ?n= and applies it to sessionCap before the default target-remainder logic |
| 2026-06-19 06:00 | FEATURE | Study tab UX corrections: (1) Captures tab redesigned as collapsible section (CaptureSection, like CurriculumSection) with Captured+Dictionary sub-rows, Imports removed; (2) Back arrows are now tab-aware — collections → /study?tab=collections, captures → /study?tab=captures, ePark → /study; (3) Study page reads ?tab= param on load; (4) ePark Saved counter shows total (not due-only) curriculum flashcards; (5) Added includeUnseen opt to listDueFlashcards, ePark Saved review uses dueOnly=false&includeUnseen=true to include rep=0 items |
| 2026-06-19 04:27 | CONFIG | update plan-v1/v+: mark cap rename done, add bugs.md ref, new study/capture TODOs, scrape candidates |
| 2026-06-19 02:00 | FIX | build: wrap StudyPage in Suspense boundary — Next.js 15 requires useSearchParams to be inside Suspense for static page generation |
...
| 2026-06-16 | FIX | ePark A2 tweaks: number+buttons header row, translation on own line, no word breaks; ActionBar opaque bottom pane replacing navbar |
| 2026-06-16 | FIX | ePark A2 polish: ActionBar taller+shadow, overflowWrap restored, zh margin 8px, blur 3px |
...
| 2026-06-16 | FIX | single layout: card now full-width (chevrons absolute-positioned in scroll padding), copy/save removed from immersive mode |
| 2026-06-16 | FEATURE | ePark D bubbles layout: odd/even alternating sides, grey audio chip, circle save at bubble bottom-inside, chat icon added |
| 2026-06-16 | FIX | bubbles polish: word wrapping, circle save button (paperHi bg + line border), grey 16px audio chip |
...
| 2026-06-16 | SCHEMA | Corpus fix (all 5 Amis dialects): deleted 510 mis-tagged twelve occurrences (66 馬蘭+66 orphan sentences; 91 南勢; 92 秀姑巒; 107 海岸; 154 恆春) — systematic scraper pagination fault mapped each Amis dialect to a wrong source dialect; full detail in docs/corpus-contamination-report.md |
| 2026-06-16 | REFACTOR | Study page: split Decks accordion into 3 sibling sub-tabs (ePark / Decks / Captures / Browser); remove collapsed state, toggleSection, EmptyTab, CustomSessionSheet (no longer reachable) |
| 2026-06-16 | FEATURE | ePark tab: curriculum rows collapsible into difficulty sub-levels; API extended with per-level completed/total (LESSON_DIFFICULTIES tiers for lessons, GRMPTS_LEVEL_NAMES levels for patterns, topic groups for essays/dialogues, flat for conversations); chevron replaced with crimson play buttons |
...
| 2026-06-14 | REFACTOR | Extract resolveEffectiveMode to CardContent.tsx; removes duplicated 7-line fallback chain from both session files |
