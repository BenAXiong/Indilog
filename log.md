...
|-----------|------|-------------|
| 2026-06-29 18:57 | FEATURE | GoalWidget: compact 3-row layout — row 1: "Goal" + deck chain (→-linked, ≤80% width, ··· if more); row 2: maturity % counters + Sim/Manual pill; row 3: progress bar |
| 2026-06-29 18:57 | FEATURE | Curriculum unit priority rows: filter_config JSONB on ind_priority_decks; matchesPriorityDeck helper; listCurriculumUnits/addCurriculumUnitDeck; GoalSheet picker shows "ePark units" section; review/learn pages use matchesPriorityDeck |
| 2026-06-29 18:57 | FIX | Backfill level/lesson/lesson_title for 77 curriculum ind_items (twelve/Lessons source) via corpus_occurrences JOIN; forward-fix EparkView.handleSave to write these fields for twelve source; add level/lesson/lesson_title to CreateItemInput |
| 2026-06-29 18:57 | REFACTOR | ePark priority decks: replace per-unit (level/lesson) approach with per-source (Lessons/Patterns/Essays/Dialogues/Conversations); collapsible ePark group in picker; filter_config now stores curriculum_source+label; matchesPriorityDeck uses level!=null as twelve proxy |
